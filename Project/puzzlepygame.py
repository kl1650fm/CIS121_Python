...
Description:
    PuzzlyWorld is a simple 3x3 sliding puzzle game.
    Solve the puzzle to see a full image and hidden message! 

Group Members : Tina Kim, Seonmin Kim, Kday Manecha Diomande
Date : 12/05/2025

Tip: try to stop the shuffle early to see the hidden message quickly!
'''
...
class GameApp:
    ...
    # Draws the intro screen with title and messages
    ...
    # Opens HiddenMessage.txt at the end of the game
    ...
    # Keeps showing the intro screen until the user presses SPACE
    def intro(self):
        running = True
        while running:
...
# 5. Main game loop
while not exit:
    # 5.1. Control FramesPerSecond (FPS)
    clock.tick(60)
    ...
    # Move titles if key pressed or shuffling
    ...
    # 5.6. Show full image and hidden message if solved
    if gameOver == True:
